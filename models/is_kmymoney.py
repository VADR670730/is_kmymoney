...
class kmn_accounts(models.Model):
    ...
    def operations_compte_action(self):
        for obj in self:
            dummy, view_id = self.env['ir.model.data'].get_object_reference('is_kmymoney', 'kmn_account_move_tree_view_editable')
            return {
                'name': obj.name,
...
class kmn_account_move(models.Model):
    ...
    def action_dupliquer(self):
        ...
        # TODO Permet de refaurchir l'écran, mais c'est lourd en code et cela allonge l'hisotrquue de navugation des pages => Mais j'ai pas trovué mieux
        if "active_id" in context:
            mod_obj = self.pool.get('ir.model.data')
            act_obj = self.pool.get('ir.actions.act_window')
            result = mod_obj.get_object_reference(cr, uid, 'is_kmymoney', 'kmn_account_move_action2')
    ...
    def action_operations_compte(self):
        context=self._context
        if "active_id" in context:
            #Ces 5 lignes permettent de retrouver le contenu de l'action 'kmn_account_move_action2'
            mod_obj = self.env['ir.model.data']
            act_obj = self.env['ir.actions.act_window']
            result = mod_obj.get_object_reference('is_kmymoney', 'kmn_account_move_action2')
            action_id = result and result[1] or False
            action=act_obj.browse(action_id)
            for obj in self:
                account_id=obj.account2_id
                if account_id.id==context["active_id"]:
                    account_id=obj.account1_id

                dict(context).update({
                    'active_id' : account_id.id,
                    'active_ids': [account_id.id],
                })


                #context["active_id"]=account_id.id
                #context["active_ids"]=[account_id.id]
                action["domain"]=['|',('account1_id','=',account_id.id),('account2_id','=',account_id.id)]
                action["context"]=context
                action["name"]=account_id.name
                return action



    def action_operations_tiers2(self):
        context=self._context
        if "active_id" in context:
            #Ces 5 lignes permettent de retrouver le contenu de l'action 'kmn_account_move_action2'
            mod_obj = self.env['ir.model.data']
            ...
            for obj in self:
                if obj.payee_id:
                    account_id=obj.account1_id
                    if account_id.id==obj.account_id.id:
                        account_id=obj.account2_id
                    dict(context).update({
                        'active_id' : account_id.id,
                        'active_ids': [account_id.id],
                    ...
                    #context["active_id"]=account_id.id
                    #context["active_ids"]=[account_id.id]
                    #context["res_id"]=account_id.id
                    action["domain"]=[('payee_id','=',obj.payee_id.id),'|',('account1_id','=',account_id.id),('account2_id','=',account_id.id)]
                    action["context"]=context
